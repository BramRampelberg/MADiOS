...
final class APIServiceHelper {
    static let shared = APIServiceHelper()
    ...
    func sendRequest<ReturnType: Decodable, BodyType: Encodable>(to url: String, method: HTTPMethod, queryParams: [String:String]? = [:], body: BodyType? = EmptyBody()) async -> Result<ReturnType> {
        if var components = URLComponents(string: "\(baseUrl.absoluteString)/\(url)"){
            if queryParams != nil {
            ...
            if let url = components.url {
                ...
                do {
                    ...
                    AppLogger.info("\(method) | \(url.absoluteString) | Status Code: \(response.statusCode)")
        ...
        return .failureWithLog(cause: "Could not parse url.", error: URLError(.badURL))
    }
    
    private func addAuthentication(to request: URLRequest) async -> Result<URLRequest>{
        do {
            var request = request
            ...
            return .failureWithLog(cause: error.cause?.localizedDescription ?? "Unexpected error while fetching the credentials.", error: error)
        } catch {
            return .failureWithLog(cause: error.localizedDescription, error: error)
        }
    }
